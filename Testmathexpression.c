...
typedef struct node{
 ...
 void parcour_prefix(node *arbre)
 {
     if(arbre==NULL)
       return;
    printf("val = %c\n",arbre->c);
    parcour_prefix(arbre->left);
    parcour_prefix(arbre->right);
 }
 ...
 void parcour_postfix(node *arbre)
 {
     if(arbre==NULL)
       return;
    parcour_postfix(arbre->left);
    parcour_postfix(arbre->right);
    printf("val = %c\n",arbre->c);
 }
...
int main(){
	...
	int i = 0;
	 ...
	 printf("Affichage du pacours prefixé:\n");
	 parcour_prefix(root);
	 printf("Affichage du pacours infixé:\n");
	 parcour_infixe(root);
	 printf("Affichage du pacours postfixé:\n");
	 parcour_postfix(root);
	return 0;
